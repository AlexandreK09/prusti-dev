...
impl<'vir: 'enc, 'enc> Enc<'vir, 'enc> {
                ...
                let phi_expr = targets.iter().zip(updates).fold(
                    ...
                    |expr, ((cond_val, _target), branch_update)| {
                        self.vcx.mk_ternary_expr(
                            self.vcx.mk_eq_expr(
                                (discr_ty_out.snap_to_prim.gen())(discr_expr),
        ...
        match rvalue {
            ...
            mir::Rvalue::UnaryOp(unop, operand) => {
                ...
                let unop_function = self
                    ...
                    .un_op()
                    .unwrap();
                unop_function.gen()(self.encode_operand(curr_ver, operand).downcast_ty())
                    .upcast_ty()
            }
            // Discriminant
            ...
            mir::Rvalue::Discriminant(place) => {
                ...
                {
                    Some(ty) => ty.unwrap().snap_to_discr_snap.gen()(
                    ...
                    None => {
                        ...
                        e_rvalue_ty.prim_to_snap.gen()(zero.upcast_ty()).lift()
                    }
                };
                discr.upcast_ty()
            }
    ...
    fn encode_place_with_ref(
        ...
        if should_wrap {
            ...
            if self.before_expiry_mode {
                expr = self.vcx.mk_old_lhs_expr(expr);
            }
        }
    ...
    fn encode_prusti_builtin(
        ...
        let prim = match builtin {
            ...
            PrustiBuiltin::Forall => {
                ...
                self.vcx.mk_forall_expr(
                    qvars,
                    &[], // TODO
                    bool.snap_to_prim.gen()(body.downcast_ty()).downcast_ty(),
                )
            }
        ...
        bool.prim_to_snap.gen()(prim.upcast_ty()).upcast_ty()
    }
}
...
fn encode_place_with_ref<'vir, 'enc>(
    ...
    let should_wrap = {
        let is_in_a_mode = self.old_mode || self.rel0_mode || self.rel1_mode || self.before_expiry_mode;
        let local_kind = self.body.local_kind(place.local);
        (local_kind == mir::LocalKind::Arg || local_kind == mir::LocalKind::ReturnPointer)
            && is_in_a_mode
    };
    ...
    match elem {
            ...
            match place_ty.ty.kind() {
                ...
                TyKind::Ref(_, inner_ty, ty::Mutability::Not) => {
                    let e_ty = deps
                        ...
                        .specifics
                        .expect_immref();
                    let val_expr = e_ty.value_access.gen()(expr);
        ...
        mir::ProjectionElem::Field(field_idx, ty) => {
            ...
            let place_ref = place_ref
                .map(|pr| struct_like.ref_to_field_refs[field_idx.as_usize()].gen()(pr, &[]));
            (proj_app, place_ref)
        }
